...
for (snp in tapas_enjoy$SNP) { #iterate through each snp with effect size
  effect_size <- tapas_enjoy$Effect_Size[tapas_enjoy$SNP == snp] # get effectsize value corresponding to the snp
  
  if (snp %in% colnames(mini_tapas)) { # check if snp is in the columns of the tapas dosage dataframe
    dosage <- list(mini_tapas[[snp]]) # pull all dosage values based on queried snp
    
    for (dose in dosage){ # calculate effect * dose for each sample
      effect_by_dose <- dose * effect_size
    }

    prs_add_value <- sum(effect_by_dose) # get sum of all effect by dose values to calc prs
    prs_all <- append(prs_all, list(prs_add_value )) #add all prs to list
...
prs_all <- unlist(prs_all, recursive = FALSE) # formatting of list
...
# appears to follow a somewhat normal distribution (wouldn't really say bimodal
#cause the dip is very small)
```
